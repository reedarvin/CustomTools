...
VOID   TCPConnect( CHAR *szTarget, DWORD *dwTCPPort, CHAR *szCommandFile, BOOL *bGetBanner, BOOL *bMultipleHosts );
VOID GetWaitValue( DWORD *dwWait );
BOOL     IsTelnet( SOCKET *s, CHAR *szRecv, DWORD *dwWait, BOOL *bDisconnect );
VOID    LogToFile( CHAR *szTarget, DWORD *dwTCPPort, CHAR *szLogText );
...
INT main( INT argc, CHAR *argv[] )
{
	...
	if ( argc > 2 && argc < 6 )
	{
		...
		printf( "Running TCPProxy v1.1 with the following arguments:\n" );
		printf( "[+] Host Input:   \"%s\"\n", szTargetInput );
		printf( "[+] TCP Port:     \"%d\"\n", dwTCPPort );
		...
		if ( argc == 4 )
		{
			...
			else
			{
				...
				strcpy( szCommandFile, szTempArg );
			...
			if ( pInputFile != NULL )
			{
				while ( fscanf( pInputFile, "%s", szReadLine ) != EOF )
				{
					...
					if ( pThreadArgs != NULL )
					{
						...
						strcpy( pThreadArgs->CommandFile, szCommandFile );
	...
	{
		printf( "TCPProxy v1.1 | Canaudit, Inc. http://www.canaudit.com/\n" );
		printf( "\n" );
		printf( "Usage: TCPProxy <ip address | ip input file> <tcp port> <command file> -nobanner\n" );
		printf( "\n" );
		printf( "<hostname | ip input file>  -- required argument\n" );
		printf( "<tcp port>                  -- required argument\n" );
		printf( "<command file               -- optional argument\n" );
		printf( "-nobanner                   -- optional argument\n" );
		printf( "\n" );
		printf( "Examples:\n" );
		printf( "TCPProxy 10.10.10.10 23\n" );
		printf( "TCPProxy 10.10.10.10 80 -nobanner\n" );
		printf( "TCPProxy 10.10.10.10 25 smtpcommands.txt\n" );
		printf( "TCPProxy IPInputFile.txt 23 ciscocommands.txt\n" );
		printf( "TCPProxy IPInputFile.txt 80 httpcommands.txt -nobanner\n" );
		printf( "\n" );
...
VOID ThreadedSub( VOID *pParameter )
{
	...
	strcpy( szCommandFile, pThreadArgs->CommandFile );
	...
	TCPConnect( szTarget, &dwTCPPort, szCommandFile, &bGetBanner, &bMultipleHosts );
		...
		if ( remoteHost != NULL )
		{
			...
			if ( s != INVALID_SOCKET )
			{
				...
				if ( ioctlsocket( s, FIONBIO, &l ) == 0 )
				{
								...
								while ( bContinue && !bDisconnect )
								{
									...
									if ( nResult > 0 )
									{
										...
										if ( !IsTelnet( &s, szRecv, &dwWait, &bDisconnect ) )
										{
											...
											LogToFile( szTarget, dwTCPPort, szRecv );
					...
					{
						while ( fgets( szCommand, sizeof( szCommand ), pCommandFile ) != NULL && !bDisconnect )
						{
							if ( szCommand[0] != '#' && szCommand[0] != '\n' )
							{
								...
								LogToFile( szTarget, dwTCPPort, szLogCommand );
					...
					else
					{
						if ( *bMultipleHosts == FALSE )
						{
							while ( !bDisconnect )
							{
								...
								sprintf( szLogSend, "\n[+] Sending %s:%d string %s\\r\\n\n", szTarget, *dwTCPPort, szTempSend );

								LogToFile( szTarget, dwTCPPort, szLogSend );

								Replace( szTempSend, "\\r", "\r" );
								Replace( szTempSend, "\\n", "\n" );
...
VOID LogToFile( CHAR *szTarget, DWORD *dwTCPPort, CHAR *szLogText )
{
	...
	sprintf( szLogFile, "LogFiles\\%s-TCPPort%d.txt", szTarget, *dwTCPPort );
