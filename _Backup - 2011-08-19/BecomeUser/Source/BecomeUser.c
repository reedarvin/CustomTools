...
INT main( INT argc, CHAR *argv[] )
{
	...
	if ( argc > 4 && argc < 7 )
	{
		...
		printf( "Running BecomeUser v1.0 with the following arguments:\n" );
	...
	{
		printf( "BecomeUser v1.0 | http://reedarvin.thearvins.com/\n" );
		printf( "\n" );
		...
		printf( "BecomeUser 10.10.10.10 administrator password cmd 1492\n" );
		printf( "\n" );
		printf( "BecomeUser MyWindowsMachine + +\"whoami > c:\\test.txt\"\n" );
		printf( "BecomeUser MyWindowsMachine administrator password cmd.exe\n" );
		printf( "BecomeUser MyWindowsMachine administrator password cmd 1492\n" );
		printf( "\n" );
...
VOID RunBecomeUser( CHAR szTarget[], CHAR szCommand[], CHAR szPID[] )
{
	...
	if ( CopyFile( "BecomeUserSvc.exe", szRemoteEXEPath, FALSE ) )
	{
		...
		{
			schService = CreateService( schSCManager, "BecomeUserSvc", "BecomeUser Service", SERVICE_ALL_ACCESS, SERVICE_WIN32_OWN_PROCESS, SERVICE_DEMAND_START, SERVICE_ERROR_IGNORE, "%windir%\\system32\\BecomeUserSvc.exe", NULL, NULL, NULL, NULL, NULL );
