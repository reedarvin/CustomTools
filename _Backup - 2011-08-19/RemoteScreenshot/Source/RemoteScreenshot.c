...
INT main( INT argc, CHAR *argv[] )
{
	...
	if ( argc == 4 )
	{
		...
		printf( "Running RemoteScreenshot v1.1 with the following arguments:\n" );
		printf( "[+] Host Input:   \"%s\"\n", szTargetInput );
		printf( "[+] Username:     \"%s\"\n", szUsername );
		printf( "[+] Password:     \"%s\"\n", szPassword );
	...
	{
		printf( "RemoteScreenshot v1.1 | http://reedarvin.thearvins.com/\n" );
		printf( "\n" );
...
VOID CaptureRemoteScreenshot( CHAR szTarget[], BOOL *bMultipleHosts )
{
	...
	if ( CopyFile( "RemoteScreenshotSvc.exe", szRemoteEXEPath, FALSE ) )
	{
		...
		{
			schService = CreateService( schSCManager, "RemoteScreenshotSvc", "RemoteScreenshot Service", SERVICE_ALL_ACCESS, SERVICE_WIN32_OWN_PROCESS | SERVICE_INTERACTIVE_PROCESS, SERVICE_DEMAND_START, SERVICE_ERROR_IGNORE, "%windir%\\system32\\RemoteScreenshotSvc.exe", NULL, NULL, NULL, NULL, NULL );
