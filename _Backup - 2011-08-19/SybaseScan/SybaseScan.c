...
VOID                                Usage( VOID );
VOID                  GuessSybaseDefaults( CHAR *szTarget, CHAR *szDictionary );
VOID                 GuessSybasePasswords( CHAR *szTarget, CHAR *szDictionary );
BOOL                                Split( CHAR *szText, CHAR *szSplitText, CHAR *szStartText, CHAR *szEndText );
VOID                          ThreadedSub( VOID *pParameter );
BOOL                        SybaseConnect( CHAR *szTarget, CHAR *szUsername, CHAR *szPassword );
VOID                  GetSybaseODBCDriver( CHAR *szODBCDriver );
	...
	if ( argc == 4 )
	{
		...
		if ( strcmp( szMethod, "-d" ) == 0 || strcmp( szMethod, "-u" ) == 0 )
		{
			if ( IsValidFile( szDictionary ) )
			{
				...
				{
					printf( "Running SybaseScan v1.0 with the following arguments:\n" );
					printf( "[+] Host Input:           \"%s\"\n", szTargetInput );
					printf( "[+] Method:               \"Sybase Defaults List\"\n" );
					printf( "[+] Sybase Defaults List: \"%s\"\n", szDictionary );
					printf( "\n" );

					GuessSybaseDefaults( szTargetInput, szDictionary );
				}
				...
				if ( strcmp( szMethod, "-u" ) == 0 )
				{
					printf( "Running SybaseScan v1.0 with the following arguments:\n" );
					printf( "[+] Host Input:        \"%s\"\n", szTargetInput );
					printf( "[+] Method:            \"Sybase Users List\"\n" );
					printf( "[+] Sybase Users List: \"%s\"\n", szDictionary );
					printf( "\n" );

					GuessSybasePasswords( szTargetInput, szDictionary );
				}
			}
		}
		else
...
VOID Usage( VOID )
{
	printf( "SybaseScan v1.0 | Canaudit, Inc. http://www.canaudit.com/\n" );
	printf( "\n" );
	printf( "Usage: SybaseScan <hostname | ip input file> [-d|-u] <dictionary file>\n" );
	printf( "\n" );
	...
	printf( "\n" );
	printf( "Examples:\n" );
	printf( "SybaseScan 10.10.10.10 -d Defaults.txt\n" );
	printf( "SybaseScan 10.10.10.10 SybaseDBInput.txt -u SybaseUsers.txt\n" );
	printf( "\n" );
	printf( "SybaseScan MySybaseMachine -d Defaults.txt\n" );
	printf( "SybaseScan MySybaseMachine -u SybaseUsers.txt\n" );
	printf( "\n" );
	printf( "SybaseScan IPInputFile.txt -d Defaults.txt\n" );
	printf( "SybaseScan IPInputFile.txt -u SybaseUsers.txt\n" );
	printf( "\n" );
	printf( "(Written by Reed Arvin | reed@canaudit.com)\n" );
}

VOID GuessSybaseDefaults( CHAR szTargetInput[], CHAR szDictionary[] )
{
	DWORD              i;
	FILE     *pInputFile;
	CHAR        szTarget[ 128 ];
...
VOID GuessSybasePasswords( CHAR szTargetInput[], CHAR szDictionary[] )
{
	DWORD             i;
	FILE    *pInputFile;
	CHAR       szTarget[ 128 ];
...
VOID ThreadedSub( VOID *pParameter )
{
	...
	if ( SybaseConnect( szTarget, szUsername, szPassword ) )
	{
		if ( strcmp( szTmpPassword, "<BLANK>" ) == 0 )
		{
			printf( "\n" );
...
BOOL SybaseConnect( CHAR szTarget[], CHAR szUsername[], CHAR szPassword[] )
{
	...
	GetSybaseODBCDriver( szODBCDriver );

	sprintf( szConnectString, "DRIVER={%s};SERVER=%s;PORT=5000;DATABASE=master;UID=%s;PWD=%s", szODBCDriver, szTarget, szUsername, szPassword );
		...
		if ( sqlReturn1 == SQL_SUCCESS || sqlReturn1 == SQL_SUCCESS_WITH_INFO )
		{
			...
			if ( sqlReturn1 == SQL_SUCCESS || sqlReturn1 == SQL_SUCCESS_WITH_INFO )
			{
				...
				if ( sqlReturn1 == SQL_SUCCESS || sqlReturn1 == SQL_SUCCESS_WITH_INFO )
				{
						...
						while ( sqlReturn1 == SQL_SUCCESS || sqlReturn1 == SQL_SUCCESS_WITH_INFO )
						{
							...
							if ( sqlReturn1 == SQL_SUCCESS || sqlReturn1 == SQL_SUCCESS_WITH_INFO )
							{
								...
								if ( ( sqlReturn2 == SQL_SUCCESS || sqlReturn2 == SQL_SUCCESS_WITH_INFO ) )
								{
									if ( i == 0 )
									{
										sprintf( szLogFileName, "%s-SybaseUserInfo.txt", szTarget );
										...
										if ( pOutputFile != NULL )
										{
											...
											fprintf( pOutputFile, "\n" );
											fprintf( pOutputFile, "Hostname\tSybase Username\n" );
					...
					if ( sqlReturn1 == SQL_SUCCESS || sqlReturn1 == SQL_SUCCESS_WITH_INFO )
					{
						strcpy( szQuery, "SELECT master..syslogins.name, master..syslogins.password FROM master..syslogins" );
						...
						while ( sqlReturn1 == SQL_SUCCESS || sqlReturn1 == SQL_SUCCESS_WITH_INFO )
						{
							...
							if ( sqlReturn1 == SQL_SUCCESS || sqlReturn1 == SQL_SUCCESS_WITH_INFO )
							{
								...
								{
									sprintf( szLogFileName, "%s-SybasePasswordHashes.txt", szTarget );
									...
									if ( i == 0 )
									{
										...
										if ( pOutputFile != NULL )
										{
											fprintf( pOutputFile, "NOTE: This file is tab separated. Open with Excel to view and sort information.\n" );
											fprintf( pOutputFile, "\n" );
											fprintf( pOutputFile, "Hostname\tSybase Username\tPassword Hash\n" );
						...
						while ( fgets( szQuery, sizeof( szQuery ), pInputFile ) != NULL )
						{
							if ( szQuery[0] != '#' && szQuery[0] != '\n' )
							{
								...
								if ( sqlReturn1 == SQL_SUCCESS || sqlReturn1 == SQL_SUCCESS_WITH_INFO )
								{
									...
									if ( sqlReturn1 == SQL_SUCCESS || sqlReturn1 == SQL_SUCCESS_WITH_INFO )
									{
										sprintf( szLogFileName, "%s-SybaseQuery%d.txt", szTarget, i + 1 );
						...
						if ( sErr != 4002 )
						{
							...
							else
							{
								...
								if ( pOutputFile != NULL )
								{
									fprintf( pOutputFile, "NOTE: This file is tab separated. Open with Excel to view and sort information.\n" );
									fprintf( pOutputFile, "\n" );
									fprintf( pOutputFile, "Hostname\tSybase Username\tPassword\tError Number\tError Message\n" );
...
VOID GetSybaseODBCDriver( CHAR szODBCDriver[] )
{
	...
	pInputFile = fopen( "SybaseScan.ini", "r" );
...
VOID LogGuessedPasswords( CHAR szTarget[], CHAR szUsername[], CHAR szPassword[] )
{
	...
	pOutputFile = fopen( "GuessedSybasePasswords.txt", "r" );
	...
	{
		pOutputFile = fopen( "GuessedSybasePasswords.txt", "w" );

		if ( pOutputFile != NULL )
		{
			fprintf( pOutputFile, "NOTE: This file is tab separated. Open with Excel to view and sort information.\n" );
			fprintf( pOutputFile, "\n" );
			fprintf( pOutputFile, "Hostname\tSybase Username\tPassword\n" );

			fclose( pOutputFile );
		}
	}

	pOutputFile = fopen( "GuessedSybasePasswords.txt", "a+" );
...
VOID LogGuessedPrivilegedAccountPasswords( CHAR szTarget[], CHAR szUsername[], CHAR szPassword[] )
{
	...
	pOutputFile = fopen( "GuessedSybasePrivilegedAccountPasswords.txt", "r" );
	...
	{
		pOutputFile = fopen( "GuessedSybasePrivilegedAccountPasswords.txt", "w" );

		if ( pOutputFile != NULL )
		{
			fprintf( pOutputFile, "NOTE: This file is tab separated. Open with Excel to view and sort information.\n" );
			fprintf( pOutputFile, "\n" );
			fprintf( pOutputFile, "Hostname\tSybase Username\tPassword\tIs Privileged Account?\n" );

			fclose( pOutputFile );
		}
	}

	pOutputFile = fopen( "GuessedSybasePrivilegedAccountPasswords.txt", "a+" );
